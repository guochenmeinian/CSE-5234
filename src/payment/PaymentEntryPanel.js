...
import { Button, FormControl, InputLabel, Input, Stack, Box, Typography, Divider } from "@mui/material";
...
function PaymentEntryPanel() {
  ...
  return (
    <ThemeProvider theme={theme}>
        <Stack direction="row" justifyContent="center" alignItems="center" spacing={2} sx={{ m: 4 }}>

        {/* Orders section */}
        <Box flex={1} p={2} border="1px solid gray" borderRadius={2}>
            <Typography variant="h6" gutterBottom>Your Orders:</Typography>
            <Divider />
            {orderData && orderData.buyQuantity.map((quantity, index) => (
            <Box key={index} mt={2}>
                <Typography>
                {productNames[index]}: {quantity} x ${productPrices[index]} = ${quantity * productPrices[index]}
                </Typography>
            </Box>
            ))}
            <Divider sx={{ mt: 2 }} />
